...
6. **Real copy, real names**: use the locked brand name, voice rules, and evocative content from the planning artifacts. Lorem or "Item 1" fails the gate.
7. **Coherent story**: pick ONE concrete content storyline (one character/product/scenario) and thread it through all screens — detail → gate → core action → status must be the same story, not disconnected samples.
