---
name: clickable-demo
description: Build a single-file clickable HTML behavior prototype containing every P0 screen with real transitions and non-happy states. Use when a portable walkthrough or deterministic transition audit is useful after screen contracts exist. Never use it as the final visual, responsive, component-reuse, or design-acceptance baseline.
---

# Clickable Demo — portable behavior contract

One self-contained HTML file makes the contracted flow portable and auditable. It proves transitions, states, and copy; it does not prove final UI fidelity or reusable implementation components. The founder's visual review surface is the React `ComponentBoard`, `DepthBoard`, and `FlowPreview` produced by `design-system-workbench`.
...
`prototypes/<product>-demo.html` + a short `04.36-clickable-demo.md` (what's wired, what's mocked, known gaps). Keep styling deliberately subordinate to the accepted React design.
...
3. **Board mode**: an optional flow-audit overview. Do not present it as the component board or final visual review.
...
- Calling the HTML demo the final UI, component board, or implementation-ready visual baseline.
